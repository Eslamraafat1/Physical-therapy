(function($) {
    ...
    window.addEventListener("load", function() {
        ...
        setTimeout(() => {
            for (let id in odometers) {
                const el = document.getElementById(id);
                if (el) {
                    el.innerHTML = odometers[id];
                }
            }
        }, 2000);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.addEventListener("DOMContentLoaded", function () {
      const interactiveItems = document.querySelectorAll('.pbm-physio-anatomy-interactive-item');
      const masterTrackerDot = document.getElementById('pbmAnatomyMasterDot');

      // لتفعيل أول عنصر تلقائياً عند تحميل الصفحة لكي لا تظهر الشاشة صامتة (كشكل جمالي احترافي)
      if(interactiveItems.length > 0) {
        activateComponent(interactiveItems[0]);
      }

      // إضافة أحداث الماوس لجميع العناصر للقوائم اليمنى واليسرى
      interactiveItems.forEach(item => {
        
        // عند تمرير الماوس فوق الكلمة (Hover In)
      ...
      // دالة التفعيل والتحريك السلس للنقطة المشعة لشغل الـ Animation المتناسق
      function activateComponent(element) {
        // إزالة الكلاس الفعال من جميع العناصر الأخرى أولاً
        interactiveItems.forEach(i => i.classList.remove('pbm-active-state'));
        
        // تفعيل العنصر الحالي
        element.classList.add('pbm-active-state');
        
        // جلب الإحداثيات المحددة بدقة من السمات (Attributes) ليرحل الماوس إليها
        const coordinateX = element.getAttribute('data-x');
        const coordinateY = element.getAttribute('data-y');
        
        // إظهار النقطة وتحريكها فوراً بالإحداثيات الجديدة بطريقة ناعمة
        masterTrackerDot.classList.remove('pbm-hidden-dot');
        masterTrackerDot.style.left = coordinateX;
        masterTrackerDot.style.top = coordinateY;
      }
    });